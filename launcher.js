...
export class launcher {
  constructor() {
    ...
    this.FuelCostPerkg =
      ((this.MassOfHydrogen / this.MassOfOneGallonOfLiquidHydrogen) *
        this.LiquidHydrogenCostPerGallon +
        (this.MassOfOxygen / this.MassOfOneGallonOfLiquidOxygen) *
          this.LiquidOxygenCostPerGallon) /
      (this.MassOfHydrogen + this.MassOfOxygen);
    this.EstimatedCostToFuelSLSToLEO =
      ((979452 - 85270 + (30710 - 3490)) * this.FuelCostPerkg) / 95000;
  ...
  Update() {
    ...
    this.PotentialEnergy_Joules =
      (-this.const_G * this.const_M * this.MPayload) / this.R_Earth;
    this.PotentialEnergy_kWh = this.PotentialEnergy_Joules / 3600000;
    this.CostOfPotentialEnergyToEscape =
      -this.PotentialEnergy_kWh * this.WholesaleElectricityCost;
    this.CostPerkgToEscape = this.CostOfPotentialEnergyToEscape / this.MPayload;
    this.LEOOrbitVelocity = Math.sqrt(
      (this.const_G * this.const_M) / (this.R_Earth + this.Alt_LEO)
    );
    this.Alt_Apogee = this.Alt_LEO;
    this.EllipseMajorAxisLength =
      this.Alt_Perigee + this.R_Earth * 2 + this.Alt_Apogee;
    this.EllipseSemiMajorAxisLength = this.EllipseMajorAxisLength / 2;
    this.Eccentricity =
      1.0 - (this.R_Earth + this.Alt_Perigee) / this.EllipseSemiMajorAxisLength;
    this.EllipseSemiMinorAxisLength =
      this.EllipseSemiMajorAxisLength * Math.sqrt(1 - this.Eccentricity ** 2);

    this.EllipticalOrbitPerigeeVelocity = Math.sqrt(
      this.const_G *
        this.const_M *
        (2 / (this.R_Earth + this.Alt_Perigee) -
          2 / this.EllipseMajorAxisLength)
    );
    this.EllipticalOrbitApogeeVelocity = Math.sqrt(
      this.const_G *
        this.const_M *
        (2 / (this.R_Earth + this.Alt_Apogee) - 2 / this.EllipseMajorAxisLength)
    );
    this.EllipticalOrbitVelocityAtLauncherExit = Math.sqrt(
      this.const_G *
        this.const_M *
        (2 / (this.R_Earth + this.Alt_EvacuatedTube) -
          1 / this.EllipseSemiMajorAxisLength)
    );
    this.EllipticalOrbitPeriod =
      2 *
      Math.PI *
      Math.sqrt(
        Math.pow(this.EllipseSemiMajorAxisLength, 3) /
          (this.const_G * this.const_M)
      );
    this.EarthsRimSpeed =
      (2 * Math.PI * (this.R_Earth + this.Alt_Perigee)) / 24 / 3600; // ToDo: This needs to be a function of where edge of ring is
    this.DeltaVeeToCircularizeOrbit =
      this.LEOOrbitVelocity - this.EllipticalOrbitApogeeVelocity;
    this.DeltaVeeToDeCircularizeOrbit = this.DeltaVeeToCircularizeOrbit; // Need this much DeltaV to return to Earth
    this.TotalDeltaV =
      this.DeltaVeeToCircularizeOrbit + this.DeltaVeeToDeCircularizeOrbit;
    this.M0OverMf = Math.exp(
      this.TotalDeltaV / (this.RocketsSpecificImpulse * this.const_g)
    );
    this.FueledVehicleMassAtApogee =
      (this.MPayload + this.RocketEnginesMass) * this.M0OverMf;
    this.FueledVehiclesKineticEnergyAtPerigee_Joules =
      0.5 *
      this.FueledVehicleMassAtApogee *
      (this.EllipticalOrbitPerigeeVelocity - this.EarthsRimSpeed) ** 2;
    this.FueledVehiclesKineticEnergyAtPerigee_kWh =
      this.FueledVehiclesKineticEnergyAtPerigee_Joules / 3600000;
    this.CostToLaunchFueledVehicle =
      this.FueledVehiclesKineticEnergyAtPerigee_kWh *
      this.LauncherEfficiency *
      this.WholesaleElectricityCost;
    ...
    this.LauncherTrackLength =
      (0.5 * (this.EllipticalOrbitPerigeeVelocity - this.EarthsRimSpeed) ** 2) /
      (this.MaxGees * this.const_g);
    this.AccelerationTime = Math.sqrt(
      (2 * this.LauncherTrackLength) / (this.MaxGees * this.const_g)
    );
    ...
    this.AllowableUpwardTurningRadius =
      this.EllipticalOrbitPerigeeVelocity ** 2 /
      ((this.MaxGees - 1) * this.const_g);
    ...
    if (this.Alt_Perigee > this.LauncherAltitude) {
      ...
      const TriangleSideA =
        this.R_Earth + this.Alt_Perigee - this.AllowableDownwardTurningRadius;
      const TriangleSideB =
        this.R_Earth +
        this.LauncherAltitude +
        this.AllowableUpwardTurningRadius;
      const TriangleSideC =
        this.AllowableUpwardTurningRadius + this.AllowableDownwardTurningRadius;
      const AngleA = Math.acos(
        (TriangleSideA ** 2 - TriangleSideB ** 2 - TriangleSideC ** 2) /
          (-2 * TriangleSideB * TriangleSideC)
      );
      const AngleB = Math.acos(
        (TriangleSideB ** 2 - TriangleSideA ** 2 - TriangleSideC ** 2) /
          (-2 * TriangleSideA * TriangleSideC)
      );
    ...
    this.CurveUpTime =
      this.CurveUpDistance / this.EllipticalOrbitPerigeeVelocity;
    this.CurveDownTime =
      this.CurveDownDistance / this.EllipticalOrbitPerigeeVelocity;
    this.TotalTimeInLaunchSystem =
      this.AccelerationTime + this.CurveUpTime + this.CurveDownTime;
  ...
  kepler_U(dt, ro, vro, a) {
    ...
    while (Math.abs(ratio) > error && n <= nMax) {
      ...
      S = this.stumpS(a * x ** 2);
      F =
        ((ro * vro) / Math.sqrt(this.mu)) * x ** 2 * C +
        (1 - a * ro) * x * x * x * S +
        ro * x -
        Math.sqrt(this.mu) * dt;
      dFdx =
        ((ro * vro) / Math.sqrt(this.mu)) * x * (1 - a * x ** 2 * S) +
        (1 - a * ro) * x ** 2 * C +
        ro;
      ratio = F / dFdx;
      x = x - ratio;
  ...
  GetAltitudeDistanceAndVelocity(CurrentTime) {
    ...
    if (CurrentTime <= this.AccelerationTime) {
      ...
      ADAndV.Altitude =
        Math.sqrt(
          (this.R_Earth +
            ...
            this.AllowableUpwardTurningRadius) **
            2 +
            this.AllowableUpwardTurningRadius ** 2 -
            2 *
              (this.R_Earth +
                this.LauncherAltitude +
                this.AllowableUpwardTurningRadius) *
              this.AllowableUpwardTurningRadius *
              Math.cos(
                (Math.max(0, CurrentTime - this.AccelerationTime) *
                  this.EllipticalOrbitPerigeeVelocity) /
                  this.AllowableUpwardTurningRadius
              )
        ) - this.R_Earth;
      ...
      ADAndV.Distance =
        this.LauncherTrackLength +
        (CurrentTime - this.AccelerationTime) *
          this.EllipticalOrbitPerigeeVelocity;
      ...
      ADAndV.Altitude =
        Math.sqrt(
          (this.R_Earth +
            ...
            this.AllowableDownwardTurningRadius) **
            2 +
            this.AllowableDownwardTurningRadius ** 2 -
            2 *
              (this.R_Earth +
                this.Alt_Perigee -
                this.AllowableDownwardTurningRadius) *
              ...
              Math.cos(
                Math.PI +
                  (Math.min(0, CurrentTime - this.TotalTimeInLaunchSystem) *
                    this.EllipticalOrbitPerigeeVelocity) /
                    this.AllowableDownwardTurningRadius
              )
        ) - this.R_Earth;
      ...
      const V0 = new THREE.Vector2(
        this.EllipticalOrbitPerigeeVelocity / 1000,
        0
      );
  ...
  GetAerodynamicDrag(CurrentAirDensity, Speed) {
    const DragForce =
      ((CoefficientOfDrag *
        VehicleCrossSectionalAreaForDrag *
        (Speed - EarthsRimSpeed) ** 2) /
        2) *
      CurrentAirDensity;
    return DragForce;
  }
