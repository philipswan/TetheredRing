...
xyzWorld = TetheredRingRefCoordSys.localToWorld(
  elevatorCableUpperAttachPnt.clone()
);
...
// planetCoordSys
if (a == 0) {
  const rayOrigin = TetheredRingRefCoordSys.localToWorld(
    elevatorCableUpperAttachPnt.clone()
  );
  const rayDirection = TetheredRingRefCoordSys.localToWorld(
    elevatorCableLowerAttachPnt.clone()
  )
    .sub(rayOrigin)
    .normalize();
  ...
  ringMarker = new THREE.Mesh(
    new THREE.SphereGeometry(0.001 * radiusOfPlanet, 32, 16),
    greenMaterial
  );
