...
export function offset_r(
  outwardOffset,
  upwardOffset,
  currentEquivalentLatitude
) {
  return (
    outwardOffset * Math.sin(currentEquivalentLatitude) +
    upwardOffset * Math.cos(currentEquivalentLatitude)
  );
}

// Returns a "delta y" in the ring-centered cylindrical coordinate space relative to the middlemost main ring
export function offset_y(
  outwardOffset,
  upwardOffset,
  currentEquivalentLatitude
) {
  return (
    outwardOffset * -Math.cos(currentEquivalentLatitude) +
    upwardOffset * Math.sin(currentEquivalentLatitude)
  );
}
...
export class commonRingVariables {
  ...
  update() {
    this.mainRingRadius =
      (this.radiusOfPlanet + this.ringFinalAltitude) *
      Math.cos(this.equivalentLatitude);
    this.constructionEquivalentLatitude = Math.acos(
      this.mainRingRadius / this.radiusOfPlanet
    );
    this.yf =
      (this.radiusOfPlanet + this.ringFinalAltitude) *
      Math.sin(this.equivalentLatitude);
    this.y0 =
      Math.sin(this.constructionEquivalentLatitude) * this.radiusOfPlanet;
    ...
    this.currentMainRingAltitude =
      Math.sqrt(this.yc ** 2 + this.mainRingRadius ** 2) - this.radiusOfPlanet;
  }
}
...
export class elevatorCarVariables {
  ...
  update() {
    ...
    this.energyRequirementPerKg =
      this.gravitationalConstant *
      1 *
      this.massOfPlanet *
      (1 / this.radiusOfPlanet -
        1 / (this.radiusOfPlanet + this.crv.currentMainRingAltitude));
    this.batterySpecificEnergy = 265; // Wh/kg
    this.driveSystemEfficiency = 0.8;
    this.batteryMassPerKg =
      this.energyRequirementPerKg /
      (this.batterySpecificEnergy * 3600 * this.driveSystemEfficiency);
    this.travelDistance =
      this.crv.currentMainRingAltitude +
      this.dParamWithUnits["transitTubeUpwardOffset"].value; // May need to subtract the altitude of the terestrial terminus
    this.maxAccelleration = 2; // m/s2
    this.maxSpeed = 200; // m/s
    this.accellerationTime = this.maxSpeed / this.maxAccelleration;
    this.accellerationDistance = Math.min(
      this.travelDistance / 2,
      0.5 * this.maxAccelleration * this.accellerationTime ** 2
    );
    this.accellerationTime = Math.sqrt(
      (2 * this.accellerationDistance) / this.maxAccelleration
    );
    this.steadySpeedDistance =
      this.crv.currentMainRingAltitude +
      this.dParamWithUnits["transitTubeUpwardOffset"].value -
      2 * this.accellerationDistance;
...
export function getElevatorCarAltitude(dParamWithUnits, crv, ecv, t) {
  ...
  accellerationProfile.push(
    new accellerationElement(
      "D",
  ...
  accellerationProfile.push(
    new accellerationElement("A", -ecv.maxAccelleration, ecv.accellerationTime)
  );
  accellerationProfile.push(
    new accellerationElement("V", -ecv.maxSpeed, ecv.steadySpeedTime)
  );
  accellerationProfile.push(
    new accellerationElement("A", ecv.maxAccelleration, ecv.accellerationTime)
  );
...
export function xyz2lla(x, y, z) {
  ...
  const e =
    Math.sqrt(Math.pow(equitorialRadiusOfEarth, 2.0) - Math.pow(b, 2.0)) /
    equitorialRadiusOfEarth;
  ...
  let N =
    Math.pow(equitorialRadiusOfEarth, 2.0) /
    Math.sqrt(
      Math.pow(equitorialRadiusOfEarth * cs, 2.0) + Math.pow(b * sn, 2.0)
    );
  let h = p / cs - N;
...
export function lla2xyz(lat, lon, alt) {
  ...
  const c =
    1 /
    Math.sqrt(
      cosLat * cosLat + (1 - flattening) * (1 - flattening) * sinLat * sinLat
    );
