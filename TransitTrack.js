...
import {simpleExtrudeGeometry} from './simpleExtrudeGeometry.js'
import { TetherGeometry } from './tethers.js';

export class transitTubeGeometry2 extends simpleExtrudeGeometry {
    ...
    const PI2 = Math.PI * 2;
    const tubeCrossSectionPoints = []
		let sides = 16
		for ( let i = 0; i < sides; i++ ) {
			const angle = ( i / sides ) * PI2;
			tubeCrossSectionPoints.push( new Vector3( Math.sin( angle ) * tubeRadius, Math.cos( angle ) * tubeRadius ) );
		}
    const tubeCrossSection = new THREE.Shape(tubeCrossSectionPoints)
    tubeCrossSection.closePath()

    const options = {extrudePath: curve}
    super(tubeCrossSection, options)
    ...
    //   extrudeTubeShape(tubeCrossSection, offset.set( outwardOffset, upwardOffset, 0 ), color1 );
...
export class mainRingTubeGeometry extends BufferGeometry {
    ...
    // Define the tube's cross section
    const tubeCrossSection = []
		let sides = 16
		for ( let i = 0; i < sides; i++ ) {
			const angle = ( i / sides ) * PI2;
			tubeCrossSection.push( new Vector3( Math.sin( angle ) * tubeRadius, Math.cos( angle ) * tubeRadius, 0 ) );
		}
    ...
    const vector1 = new Vector3();
      ...
      extrudeTubeShape(tubeCrossSection, offset.set( 0, mainRingOffset, 0 ), color1 );
			prevPoint.copy( point );
		}
