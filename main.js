...
const massOfPlanet = 5.97e24; // kg   using mass of Earth for now
...
const guidParamWithUnits = {
  ...
  elevatorUpperTerminusOutwardOffset: {
    value: -30,
    units: "m",
    autoMap: true,
    min: -100,
  ...
  dParamWithUnits["ringCenterLongitude"] = {
    value:
      (tram.lerp(
        ...
        guidParamWithUnits["moveRing"].value
      ) /
        180) *
      Math.PI,
    units: "radians",
...
const metalicMaterial = new THREE.MeshLambertMaterial({
  color: 0x878681,
  transparent: false,
});
...
tetheredRingLonCoordSys.rotation.y =
...
tetheredRingLatCoordSys.rotation.x =
  -dParamWithUnits["ringCenterLatitude"].value;
...
function constructMainRingCurve() {
  ...
  for (
    let a = 0, i = 0;
    i < dParamWithUnits["numControlPoints"].value;
    a += (Math.PI * 2) / dParamWithUnits["numControlPoints"].value, i++
  ) {
    const angleInRingCoordSys =
      Math.acos(
        crv.mainRingRadius / (radiusOfPlanet + crv.currentMainRingAltitude)
      ) * Math.sqrt((e * Math.cos(a)) ** 2 + ((1 / e) * Math.sin(a)) ** 2);
    const rInRingCoordSys =
      (radiusOfPlanet + crv.currentMainRingAltitude) *
      Math.cos(angleInRingCoordSys);
    const yInRingCoordSys =
      (radiusOfPlanet + crv.currentMainRingAltitude) *
      Math.sin(angleInRingCoordSys);
    const xInRingCoordSys = rInRingCoordSys * Math.cos(a);
    const zInRingCoordSys = rInRingCoordSys * Math.sin(a);
    controlPoints.push(
      new THREE.Vector3(xInRingCoordSys, yInRingCoordSys, zInRingCoordSys)
    );
  }
...
function constructMainRings() {
  ...
  for (let j = 0; j < numWedges; j++) {
    ...
    for (let i = 0; i < dParamWithUnits["numMainRings"].value; i++) {
      const mainRingGeometry = new mainRingTubeGeometry(
        ringCurve,
        start,
        end,
        referencePoint,
        8192 / numWedges,
        (i - mro) * dParamWithUnits["mainRingSpacing"].value,
        dParamWithUnits["mainRingTubeRadius"].value
      );
...
function constructTransitSystem() {
  ...
  for (let i = 0; i < trackOffsetsList.length; i++) {
    outwardOffset[i] =
      dParamWithUnits["transitTubeOutwardOffset"].value +
      trackOffsetsList[i][0] * dParamWithUnits["transitTubeTubeRadius"].value;
    upwardOffset[i] =
      dParamWithUnits["transitTubeUpwardOffset"].value +
      trackOffsetsList[i][1] * dParamWithUnits["transitTubeTubeRadius"].value;
  }
  for (let j = 0; j < numWedges; j++) {
    start = j / numWedges;
    end = (j + 1) / numWedges;
    ...
    for (let i = 0; i < dParamWithUnits["numMainRings"].value; i++) {
      const mainRingGeometry = new transitTubeGeometry(
        ringCurve,
        start,
        end,
        referencePoint,
    ...
    const tubeGeometry = new transitTubeGeometry(
      ringCurve,
      start,
      end,
      referencePoint,
...
if (
  dParamWithUnits["showTransitSystem"].value &&
  dParamWithUnits["showTransitVehicles"].value
) {
...
function constructTransitVehicles() {
  ...
  specs["transitVehiclePowerConsumptionWhenCruising"] = {
    value: (F * V) / η,
    units: "W",
  };
  ...
  function addTransitVehicles({ scene }) {
    ...
    object.scale.set(0.034, 0.034, 0.034);
    //object.visible = false
    for (
      let a = 0, i = 0;
      i < dParamWithUnits["numTransitVehicles"].value;
      a += (Math.PI * 2) / dParamWithUnits["numTransitVehicles"].value, i++
    ) {
      for (let i = 0; i < trackOffsetsList.length; i++) {
        const outwardOffset =
          dParamWithUnits["transitTubeOutwardOffset"].value +
        ...
        const upwardOffset =
          ...
          trackOffsetsList[i][1] *
            dParamWithUnits["transitTubeTubeRadius"].value -
          dParamWithUnits["transitVehicleRadius"].value -
          0.35; // Last is half of the track height

        transitVehicleMeshes.push(
          makeTransitVehicleMesh(object, outwardOffset, upwardOffset, a, i)
        );
      }
    }
    ...
    function makeTransitVehicleMesh(object, outwardOffset, upwardOffset, a, i) {
      ...
      computeTransitVehiclePositionAndRotation(
        transitVehicleMesh,
        outwardOffset,
        upwardOffset,
        a,
...
function computeTransitVehiclePositionAndRotation(
  transitVehicleMesh,
  outwardOffset,
  upwardOffset,
  a,
  ...
  const transitVehiclePosition_r =
    crv.mainRingRadius +
    tram.offset_r(outwardOffset, upwardOffset, crv.currentEquivalentLatitude);
  const a2 = a + d / transitVehiclePosition_r;
  transitVehicleMesh.position.set(
    transitVehiclePosition_r * Math.cos(a2),
    crv.yc +
      tram.offset_y(outwardOffset, upwardOffset, crv.currentEquivalentLatitude),
    transitVehiclePosition_r * Math.sin(a2)
  );
...
function constructLaunchTube() {
  ...
  const launchTubeRadius =
    crv.mainRingRadius +
    tram.offset_r(
      launchTubeOutwardOffset,
      -dParamWithUnits["launchTubeUpwardOffset"].value,
      crv.currentEquivalentLatitude
    );
  const launchTube_y =
    crv.yc +
  ...
  const launchTubeArc =
    (dParamWithUnits["launchTubeLength"].value /
      (2 * Math.PI * launchTubeRadius)) *
    2 *
    Math.PI;
  ...
  const launchTubeGeometry = new THREE.TorusGeometry(
    launchTubeRadius,
    launchTubeTubeRadius,
    launchTubeRadialSegments,
    launchTubeTubularSegments,
    launchTubeArc
  );
  const launchTubeMesh = new THREE.Mesh(
    launchTubeGeometry,
    transparentMaterial1
  );
  ...
  function makeTrackMesh(
    ...
    height,
    launchTubeRadius,
    launchTubePosition_y,
    currentEquivalentLatitude
  ) {
    const trackInnerRadius =
      launchTubeRadius +
      tram.offset_r(
        outwardOffset - width / 2,
        upwardOffset - height / 2,
        crv.currentEquivalentLatitude
      );
    ...
    const trackGeometry = new THREE.TorusGeometry(
      (trackInnerRadius + trackOuterRadius) / 2,
      width,
      8,
      thetaSegments,
      launchTubeArc
...
function constructElevatorCables() {
  ...
  const cableOutwardOffset =
    dParamWithUnits["transitTubeOutwardOffset"].value -
    dParamWithUnits["transitTubeTubeRadius"].value +
    dParamWithUnits["elevatorUpperTerminusOutwardOffset"].value;
  const elevatorCableUpperAttachPnt_r =
    crv.mainRingRadius +
    tram.offset_r(
      cableOutwardOffset,
      dParamWithUnits["transitTubeUpwardOffset"].value +
  ...
  const elevatorCableupperElevatorTerminus_r =
    crv.mainRingRadius +
    tram.offset_r(
      cableOutwardOffset,
      dParamWithUnits["transitTubeUpwardOffset"].value,
  ...
  const elevatorCableLowerAttachPnt_r =
    crv.mainRingRadius +
    tram.offset_r(
      cableOutwardOffset,
      -crv.currentMainRingAltitude,
  ...
  const elevatorCableUpperAttachPnt_y =
    crv.yc +
    tram.offset_y(
      cableOutwardOffset,
      dParamWithUnits["transitTubeUpwardOffset"].value +
  ...
  const elevatorCableupperElevatorTerminus_y =
    crv.yc +
    tram.offset_y(
      cableOutwardOffset,
      dParamWithUnits["transitTubeUpwardOffset"].value,
  ...
  const elevatorCableLowerAttachPnt_y =
    crv.yc +
    tram.offset_y(
      cableOutwardOffset,
      -crv.currentMainRingAltitude,
  ...
  let tempGeometry;
  const upperElevatorTerminusLength =
    2 * -dParamWithUnits["elevatorUpperTerminusOutwardOffset"].value;
  const upperElevatorTerminusWidth = 20;
  const upperElevatorTerminusThickness = 10;
  const upperElevatorTerminusMesh = new THREE.Mesh(
    new THREE.BoxGeometry(
      upperElevatorTerminusLength,
      upperElevatorTerminusThickness,
      upperElevatorTerminusWidth
    ),
    transparentMaterial2
  );
  ...
  for (let a = 0, i = 0; i < n; a += (Math.PI * 2) / n, i++) {
    ...
    tempGeometry = new THREE.BufferGeometry().setFromPoints([
      elevatorCableUpperAttachPnt.sub(elevatorCableReferencePnt),
      elevatorCableLowerAttachPnt.sub(elevatorCableReferencePnt),
    ]);
    ...
    upperElevatorTerminusMesh.rotation.z =
      crv.currentEquivalentLatitude - Math.PI / 2;
    upperElevatorTerminusMesh.position.set(
      elevatorCableupperElevatorTerminus.x,
      elevatorCableupperElevatorTerminus.y,
      elevatorCableupperElevatorTerminus.z
    );
    ...
    upperElevatorTerminusMesh.position.set(
      elevatorCableLowerAttachPnt.x,
      elevatorCableLowerAttachPnt.y,
      elevatorCableLowerAttachPnt.z
    );
    elevatorCableMeshes.push(upperElevatorTerminusMesh.clone());
  }
...
function constructElevatorCars() {
  ...
  function addCars(object) {
    ...
    const cableOutwardOffset =
      ...
      dParamWithUnits["transitTubeTubeRadius"].value +
      dParamWithUnits["elevatorUpperTerminusOutwardOffset"].value;
    const elevatorCarPosition_r =
      crv.mainRingRadius +
      tram.offset_r(
        cableOutwardOffset,
        elevatorAltitude - crv.currentMainRingAltitude,
        crv.currentEquivalentLatitude
      );
    const elevatorCarPosition_y =
      crv.yc +
      tram.offset_y(
        cableOutwardOffset,
        elevatorAltitude - crv.currentMainRingAltitude,
    ...
    for (
      let a = 0, i = 0;
      i < dParamWithUnits["numElevatorCars"].value;
      a += (Math.PI * 2) / dParamWithUnits["numElevatorCars"].value, i++
    ) {
      ...
      // Add elevator car
      elevatorCarMesh.position.set(
        elevatorCarPosition.x,
        elevatorCarPosition.y,
        elevatorCarPosition.z
      );
...
function constructTethers() {
  ...
  function tetherMath() {
    ...
    const fExertedByGravityOnRing =
      (gravitationalConstant * massOfPlanet * m) / final_r ** 2;
    ...
    fG.ρ =
      -fExertedByGravityOnRing *
      Math.cos(dParamWithUnits["equivalentLatitude"].value);
    fG.φ = 0;
    fG.z =
      -fExertedByGravityOnRing *
      Math.sin(dParamWithUnits["equivalentLatitude"].value);
    fT.z = -fG.z; // Eq 6

    const tetherStress =
      (dParamWithUnits["tetherMaterialTensileStrength"].value * 1000000) /
      dParamWithUnits["tetherEngineeringFactor"].value;
    const aveForceOfGravity =
      (gravitationalConstant * massOfPlanet * 1) /
      (radiusOfPlanet + dParamWithUnits["ringFinalAltitude"].value / 2) ** 2;
    const c =
      tetherStress /
      dParamWithUnits["tetherMaterialDensity"].value /
      aveForceOfGravity; // We're using the average force of gravity here as an engineering approximation (Eq 17)
    ...
    const numTetherSegments =
      2 ** (dParamWithUnits["numForkLevels"].value + 1) - 1; // Starting from anchor, after each fork the distance to the next fork (or attacment point) is halved
    const numTetherPoints = numTetherSegments + 1; // Because, for example, it takes 5 points to speify 4 segments

    specs["tetherSpacing"] = {
      value:
        (2 * crv.mainRingRadius * Math.PI) /
        dParamWithUnits["numTethers"].value,
      units: "m",
    };
    ...
    finalCatenaryTypes.forEach((catenaryType, j) => {
      ...
      pointB.x =
        (tempPointP.x *
          (dParamWithUnits["tetherPointBxAvePercent"].value +
            (minusplus[j] *
              dParamWithUnits["tetherPointBxDeltaPercent"].value) /
              2)) /
        100;
      ...
      points.forEach((point, k) => {
        point.crossSectionalArea =
          (point.T / tetherStress) * Math.cosh(point.s / c); // Eq 14
        ...
        specs["numAnchorPoints_" + label[k] + j] = {
          value: 2 ** (k * dParamWithUnits["numForkLevels"].value),
          units: "",
        };
        specs["tetherCrossSectionalArea_" + label[k] + j] = {
          value:
            (point.crossSectionalArea * specs["tetherSpacing"].value) /
            specs["numAnchorPoints_" + label[k] + j].value,
          units: "m2",
        };
        specs["tetherDiameter_" + label[k] + j] = {
          value:
            2 *
            Math.sqrt(
              specs["tetherCrossSectionalArea_" + label[k] + j].value /
                2 /
                Math.PI
            ),
          units: "m",
        }; // because: d = 2*r = 2*sqrt(a/2/pi)
        specs["tetherForce_" + label[k] + j] = {
          value:
            specs["tetherCrossSectionalArea_" + label[k] + j].value *
            tetherStress,
          units: "N",
      ...
      specs["tetherLength_Rough" + j] = {
        value: pointP.s - pointB.s,
        units: "m",
      }; // Note: Does not account for forks
      specs["tetherVolume_Rough" + j] = {
        value:
          (specs["tetherLength_Rough" + j].value *
            (specs["tetherCrossSectionalArea_" + label[0] + j].value *
              specs["numAnchorPoints_" + label[0] + j].value +
              specs["tetherCrossSectionalArea_" + label[1] + j].value *
                specs["numAnchorPoints_" + label[1] + j].value)) /
          2,
        units: "m3",
      };
      specs["tetherMass_Rough" + j] = {
        value:
          specs["tetherVolume_Rough" + j].value *
          dParamWithUnits["tetherMaterialDensity"].value,
        units: "kg",
      };
    });
    specs["tetherMaterialTotalMass_Rough"] = {
      value:
        ((specs["tetherMass_Rough" + 0].value +
          specs["tetherMass_Rough" + 1].value) /
          2) *
        dParamWithUnits["numTethers"].value,
      units: "kg",
    };
    specs["tetherEqCO2TotalMass_Rough"] = {
      value: (specs["tetherMaterialTotalMass_Rough"].value * 44) / 12,
      units: "kg",
    };
    specs["tetherCost_Rough"] = {
      value:
        (specs["tetherMaterialTotalMass_Rough"].value *
          dParamWithUnits["tetherMaterialCost"].value) /
        1000000000,
      units: "Billion USD",
    };
    ...
    const r = Math.sqrt(
      crv.yc * crv.yc + crv.mainRingRadius * crv.mainRingRadius
    );
    ...
    currentCatenaryTypes.forEach((catenaryType, j) => {
      ...
      for (let i = 0; i <= numTetherPoints - 1; i++) {
        ...
        const s =
          pointB.s +
          currentTetherLength[j] -
          finalTetherLength[j] * (1 - sFraction);
    ...
    function makeTetherStrips(j, θ, referencePoint) {
      ...
      let branches = [];
      branches.push(
        new Branch(0, 0.0, 0.0, 0.0, numTetherPoints, 0.0, 0.0, 0.0)
      ); // This defines the trunk of the tether
      ...
      for (let i = 0; i <= numTetherPoints - 2; i++) {
        ...
        if (i > 0 && Number.isInteger(Math.log2(numTetherPoints - i))) {
          // If we're at a point where the tether segments fork...
          const logNumStays =
            dParamWithUnits["numForkLevels"].value +
            1 -
            Math.log2(numTetherPoints - i);
          const tetherSpan =
            ((2 * Math.PI) / dParamWithUnits["numTethers"].value) *
            dParamWithUnits["tetherSpanOverlapFactor"].value;
            ...
            if (i < numTetherPoints - 2) {
              ...
              branches.push(
                new Branch(
                  ...
                  branch.target_dr,
                  branch.target_dω,
                  branch.target_dθ + target_dθ_Alteration
                )
              );
              branches.push(
                new Branch(
              ...
              for (let k = 0; k < dParamWithUnits["numMainRings"].value; k++) {
                const target_dr_Alteration =
                  (k - mro) *
                  dParamWithUnits["mainRingSpacing"].value *
                  Math.sin(crv.constructionEquivalentLatitude);
                const target_dω_Alteration =
                  ((k - mro) *
                    dParamWithUnits["mainRingSpacing"].value *
                    Math.cos(crv.constructionEquivalentLatitude)) /
                  r_0; // Dividing by r_0 is an approximation, a better r value may be needed for perfect accuracy
                branches.push(
                  new Branch(
                    i,
                    branch.dr_0,
                    ...
                    branch.dθ_0,
                    numTetherPoints,
                    branch.target_dr + target_dr_Alteration,
                    branch.target_dω + target_dω_Alteration,
                    branch.target_dθ + target_dθ_Alteration
                  )
                );
                branches.push(
        ...
        branches.forEach((branch) => {
          const alpha =
            (i + 1 - branch.base_point) /
            (numTetherPoints - 1 - branch.base_point);
          ...
          if (s_1 > pointB_s[jModNumTypes]) {
            // When raising the ring, points on the parts of the tether that are on the spool have all have the same coordinates (i.e. the spool's coordinates).
            ...
            if (branch.stripIndex == -1) {
              ...
              const absolutePoint = new THREE.Vector3().setFromSphericalCoords(
                r_0 + branch.dr_0,
                ω_0 + branch.dω_0,
                θ + branch.dθ_0
              );
              tetherStrips[branch.stripIndex].push(
                absolutePoint.sub(referencePoint)
              );
            }
            const absolutePoint = new THREE.Vector3().setFromSphericalCoords(
              r_1 + branch.dr_1,
...
function updateRing() {
  ...
  if (majorRedesign) {
    ...
    mainRingMeshes.forEach((mesh, i) => {
      mesh.position.y =
        crv.yc +
        (i - (dParamWithUnits["numMainRings"].value - 1) / 2) *
          dParamWithUnits["mainRingSpacing"].value;
    });
  }
...
function renderFrame() {
  ...
  if (animateRingRaising || animateRingLowering) {
    ...
    if (animateRingRaising) {
      guidParamWithUnits["ringAmountRaisedFactor"].value = Math.min(
        1,
        guidParamWithUnits["ringAmountRaisedFactor"].value + delta * 0.01
      );
  ...
  if (animateCameraGoingUp || animateCameraGoingDown) {
    ...
    if (animateCameraGoingDown) {
      ...
      if (camera.position.length() <= radiusOfPlanet)
        animateCameraGoingDown = false;
    }
  }

  if (
    dParamWithUnits["showElevators"].value &&
    dParamWithUnits["animateElevators"].value
  ) {
    elevatorAltitude = tram.getElevatorCarAltitude(
      dParamWithUnits,
      crv,
      ecv,
      timeSinceStart
  ...
  if (
    dParamWithUnits["showTransitSystem"].value &&
    dParamWithUnits["showTransitVehicles"].value &&
    dParamWithUnits["animateTransitVehicles"].value
  ) {
    const TransitVehiclePosition = tram.getTransitVehiclePosition(
      dParamWithUnits,
      crv,
      tvv,
      timeSinceStart
    ...
    case 80 /*P*/:
      ...
      if (transitTubeIntersects.length > 0) {
        ...
        targetPoint = intersectionPoint.multiplyScalar(
          (radiusOfPlanet + crv.currentMainRingAltitude) / radiusOfPlanet
        );
      ...
      if (planetIntersects.length > 0 || transitTubeIntersects.length > 0) {
        ...
        orbitControlsNewMaxPolarAngle = Math.PI / 2 + 0.1;
      }
      break;
    case 81 /*Q*/:
      orbitControls.autoRotate ^= true;
...
function recomputeNearFarClippingPlanes() {
  ...
  camera.near =
    Math.max(
      10,
      camera.position.distanceTo(planetMeshes[0].position) -
        (radiusOfPlanet +
          dParamWithUnits["ringFinalAltitude"].value +
  ...
  const d1Squared =
    camera.position.distanceTo(planetMeshes[0].position) ** 2 -
    radiusOfPlanet ** 2;
  ...
  //}
    ...
    if (!toRingAlreadyTriggered) {
      ...
      const intersection = (mesh) => {
        ...
        return raycaster.ray.intersectSphere(
      ...
      if (distanceToPointOnEarthsSurface < innerTransitionDistance) {
        ...
        const numGoodSpots = Math.min(
          dParamWithUnits["numTransitVehicles"].value,
          dParamWithUnits["numElevatorCables"].value
        );
        const roundedTheta =
          (Math.round((originalTheta / (Math.PI * 2)) * numGoodSpots) /
            numGoodSpots) *
          Math.PI *
          2;
