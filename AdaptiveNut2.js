...
export class launchSledModel {
  constructor(dParamWithUnits, myScene, unallocatedModelsList, perfOptimizedThreeJS, massDriverScrewTexture) {
    ...
    const grapplerLength = dParamWithUnits['launchSledGrapplerLength'].value
    const launchSledForwardsOffset = dParamWithUnits['launchSledForwardsOffset'].value
    ...
    launchSledBodyGeometry.translate(0, bodyLength/2 - (bodyLength-grapplerLength)/2 - launchSledForwardsOffset, 0)
    ...
    function addGrapplers(launchSledMesh) {
      ...
      midwayPositionList.forEach(midwayPosition => {
        ...
        for (let i = 0, grapplerDistance = firstGrapplerDistance; grapplerDistance<lastGrapplerDistance; i++, grapplerDistance += grapplerSpacing) {
          ...
          const launchSledGrapplerMesh = createSledGrapplerMesh(dParamWithUnits, distanceToSledAft, grapplerLength, midwayRotation, grapplerDistance, massDriverScrewTexture)
    ...
    function createSledGrapplerMesh(dParamWithUnits, distanceToSledAft, grapplerLength, midwayRotation, grapplerDistance, massDriverScrewTexture) {
      ...
      const initialDistance = dParamWithUnits['launchSledGrapplerLength'].value / 2
      const numGrapplers = dParamWithUnits['launchSledNumGrapplers'].value
...
export class virtualLaunchSled {
    ...
    static update(dParamWithUnits, launcherMassDriverLength, scene, clock) {
      ...
      virtualLaunchSled.launchSledGrapplerLength = dParamWithUnits['launchSledGrapplerLength'].value
      ...
      virtualLaunchSled.initialDistance = dParamWithUnits['launchSledGrapplerLength'].value / 2
    ...
    placeAndOrientModel(om, refFrame) {
      if (virtualLaunchSled.isVisible) {
        ...
        const grapplerLength = virtualLaunchSled.launchSledGrapplerLength
